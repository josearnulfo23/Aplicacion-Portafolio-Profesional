 ...
 * v2.0.1: Fix caché bloqueante — invalidarCache() + App.inicializar hacen hard-reload.
 *         También se añade getBundle() y contacto usa el perfil persistido.
 * ==========================================================================
 */
...
export class RepositorioContenido {
  ...
  static getBundle() { return this._bundle; }

  static async cargarTodo() {
    const esNode = typeof window === 'undefined' || typeof window.document === 'undefined';
    if (!esNode) {
      try {
        const response = await fetch('/api/content/all', { cache: 'no-store' });
        if (response.ok) {
  ...
  static async cargarJSON(rutaRelativa) {
    ...
    try {
      const response = await fetch(rutaRelativa, { cache: 'no-store' });
      if (!response.ok) throw new Error(`Error HTTP ${response.status} al cargar ${rutaRelativa}`);
  ...
  static async cargarPerfil() {
    if (this._bundle && this._bundle.profile) return new PerfilProfesional(this._bundle.profile);
    const raw = await this.cargarJSON("src/js/data/profile.json");
    return new PerfilProfesional(raw || {});
  }

  static async cargarBiografia() {
    if (this._bundle && this._bundle.about) return new Biografia(this._bundle.about);
  ...
  static async cargarHabilidades() {
    if (this._bundle && this._bundle.skills) return this._bundle.skills.map(cat => new CategoriaHabilidad(cat));
  ...
  static async cargarProyectos() {
    if (this._bundle && this._bundle.projects) return this._bundle.projects.map(p => new Proyecto(p));
    const raw = await this.cargarJSON("src/js/data/projects.json");
    return (raw || []).map(p => new Proyecto(p));
  }

  static async cargarServicios() {
    if (this._bundle && this._bundle.services) return this._bundle.services.map(s => new Servicio(s));
    const raw = await this.cargarJSON("src/js/data/services.json");
    return (raw || []).map(s => new Servicio(s));
  }

  static async cargarEducacion() {
    if (this._bundle && this._bundle.education) return new EducacionYCertificacion(this._bundle.education);
  ...
  static async cargarTestimonios() {
    if (this._bundle && this._bundle.testimonials) return this._bundle.testimonials.map(t => new Testimonio(t));
    const raw = await this.cargarJSON("src/js/data/testimonials.json");
    return (raw || []).map(t => new Testimonio(t));
  }

  static invalidarCache() { this._bundle = null; }
}
